...
%
fprintf('Extracting knickpoints for all streams. At basin # of %d: ', number_of_basins);
for i = 1:number_of_basins
    fprintf('%d, ', i);
    if mod(i,10) == 0
        fprintf('\nExtracting knickpoints for all streams. At basin # of %d: ', number_of_basins);
    end
    ...
    for j = 1:length(tributaries);
        ...
        if length(chi_stored{j}) < min_trib_size ; % measure the length of the chi vector for each tributary. if that vector is too small we ignore that stream segment
            chi_stored{j} = -9999 ; % some ridiculous number you can use as a marker indicating a cell that refers to a stream that is too small that we don't care about
            elev_stored{j} = -9999;
            eastingUTM11_stored{j} = -9999;
...
fprintf('Postprocessing knickpoints for all tributaries. At basin # of %d: ', ...
    number_of_basins);
for i = 1:number_of_basins
    fprintf('%d, ', i);
    if mod(i,10) == 0
        fprintf('\nPostprocessing knickpoints for all tributaries. At basin # of %d: ', ...
            number_of_basins);
    end
...
%         end
        ...
        kp_chi_length_final{j} = kp_length_chi(idx_kp_chi_length );                       % respective knickpoint lengths (in units of chi)
        
        idx_kp_face_slope_dist_space_detrended  = ~isnan(kp_face_slope_chi);
        kp_face_slope_chi_final{j}= kp_face_slope_chi(idx_kp_face_slope_dist_space_detrended);   % respective knickpoint slopes  detrended elev/chi
...
txt = sprintf('%s, ',hdr{:});
txt(end)='';
kp_lips_fn = strcat(DEM_basename_nodir, '_kp_lips.csv');
dlmwrite(kp_lips_fn,txt,'');
dlmwrite(kp_lips_fn,Knickpoint_Database_Lips,'-append','delimiter',',', 'precision', '%.5f');

kp_bases_fn = strcat(DEM_basename_nodir, '_kp_bases.csv');
...
% formats with gdal
kp_lips_shapeout_fn = strcat(DEM_basename_nodir, '_kp_lips.shp');
kp_lips_shapeout_fn_out = strcat(DEM_basename_nodir, '_kp_lips.out');
kp_lips_shapeout_fn_all = strcat(DEM_basename_nodir, '_kp_lips.*');
kp_lips_crt_fn = strcat(DEM_basename_nodir, '_kp_lips.crt');
...
string2write = sprintf(['<OGRVRTDataSource>\n  <OGRVRTLayer name=\"%s\">\n', ...
    ...
    '  </OGRVRTLayer>\n', '</OGRVRTDataSource>\n'], strcat(DEM_basename_nodir, '_kp_lips'), ...
    kp_lips_fn);
...
kp_bases_shapeout_fn = strcat(DEM_basename_nodir, '_kp_bases.shp');
kp_bases_shapeout_fn_out = strcat(DEM_basename_nodir, '_kp_bases.out');
kp_bases_shapeout_fn_all = strcat(DEM_basename_nodir, '_kp_bases.*');
kp_bases_crt_fn = strcat(DEM_basename_nodir, '_kp_bases.crt');
...
string2write = sprintf(['<OGRVRTDataSource>\n  <OGRVRTLayer name=\"%s\">\n', ...
    ...
    '    <Field name="22strea_sz" type="Real" width="8" precision="7"/>\n', ...
    '  </OGRVRTLayer>\n', '</OGRVRTDataSource>\n'], ...
    strcat(DEM_basename_nodir, '_kp_bases'), kp_bases_fn);
