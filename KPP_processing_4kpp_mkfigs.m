...
if show_figs == 1 || show_figs == 2
    ...
    for i = 1:number_of_basins2
        fprintf('%d, ', i);
        knickpoints_plot_fname = sprintf('plots/%s_basin_%d_knickpoints.pdf', DEM_basename_nodir, i);
        ...
        if ~isempty(sig_kps_magnitude_all_tribs_all_kp_size_current_basin)
            ...
            if sig_kps_magnitude_all_tribs_all_kp_size_current_basin_max > max_knickpointsize2plot
                %scale knickpoint for plotting
                kp_size2plot = sig_kps_magnitude_all_tribs_all_kp_size_current_basin_max/max_knickpointsize2plot;
            elseif sig_kps_magnitude_all_tribs_all_kp_size_current_basin_max < max_knickpointsize2plot
                kp_size2plot = sig_kps_magnitude_all_tribs_all_kp_size_current_basin_max/max_knickpointsize2plot;
            else
                kp_size2plot = 1;
            end
        ...
        if ~isempty(sig_kps_chi_all_tribs_all_kp_size_current_basin_t)
            ...
            for k = 1:length(sig_kps_magnitude_tribs_current_trib_t);  % plot each knickpoint seperately ( # of kps in that trib = length of sig_kps_magnitude_tribs_current_trib )
                plot(sig_kps_chi_tribs_current_trib_t(k),sig_kps_elevation_tribs_current_trib_t(k),'r.','MarkerSize',(sig_kps_magnitude_tribs_current_trib_t(k)/kp_size2plot)) % size of knickpoint marker is relative to kp_magnitude value
                plot(troughs_chi_tribs_current_trib_t(k),troughs_elevation_tribs_current_trib_t(k),'bs','MarkerSize',(sig_kps_magnitude_tribs_current_trib_t(k)/kp_size2plot)) % size of knickpoint marker is relative to kp_magnitude value
            end
            hold off
        end
        
        subplot(2,2,3) % spatial plot on regional hillshade (last figure after all the basin individual figures are done)
        hold on
        for k = 1:length(sig_kps_magnitude_tribs_current_trib_t);% plot each knickpoint seperately ( # of kps in that trib = length of sig_kps_magnitude_tribs_current_trib )
            plot(sig_kps_easting_tribs_current_trib_t(k),sig_kps_northing_tribs_current_trib_t(k),'r.','MarkerSize',(sig_kps_magnitude_tribs_current_trib_t(k)/kp_size2plot)) % same marker for kps as above
        end
        hold off
        ...
        hold on
        for k = 1: length(sig_kps_magnitude_tribs_current_trib_t);  % plot each knickpoint seperately ( # of kps in that trib = length of sig_kps_magnitude_tribs_current_trib )
            plot(sig_kps_distance_tribs_current_trib_t(k), sig_kps_elevation_tribs_current_trib_t(k), 'r.','MarkerSize',(sig_kps_magnitude_tribs_current_trib_t(k)/kp_size2plot)); % size of knickpoint marker is relative to kp_magnitude value
        end
        ...
        for j = 1:length(sig_kps_chi_all_tribs_all_kp_size_current_basin);
            ...
            for k = 1:length(sig_kps_magnitude_tribs_current_trib);% plot each knickpoint seperately ( # of kps in that trib = length of sig_kps_magnitude_tribs_current_trib )
                ...
                if (sig_kps_magnitude_tribs_current_trib(k)/5) < 100
                    plot(sig_kps_easting_tribs_current_trib(k),sig_kps_northing_tribs_current_trib(k),'r.','MarkerSize',(sig_kps_magnitude_tribs_current_trib(k)/kp_size2plot)) % same marker for kps as above            %plot(troughs_easting_tribs_current_trib(k),troughs_northing_tribs_current_trib(k),'k.','MarkerSize',8) % same marker for kps as above
                end
            end
        ...
        knickpoints_plot_fname = sprintf('plots/%s_basin_%d_knickpoints.pdf', DEM_basename_nodir, i);
        if exist(knickpoints_plot_fname, 'file') ~= 2
            export_fig(knickpoints_plot_fname,quality_flag,'-pdf');
        end
        close all
    end
end
fprintf('\nfinished with KPP_processing_v1.m\n');
