...
fprintf('step 3 of 4: area-slope and chi-plot processing for all basins draining to edge\n');
if exist('AOI_DEM', 'var') ~= 1
    load(DEM_MAT_fname)
end
if exist('AOI_dbasins', 'var') ~= 1
    load(DEM_HYD_MAT_fname, 'AOI_dbasins', 'AOI_FAC', 'AOI_FD', 'minApix', ...
        'AOI_DEM_gradient8', 'AOI_mg', 'AOI_STR_MS')
end
if exist(DEM_STR_MAT_fname, 'file') == 0 || show_figs == 1 || show_figs == 2
    if exist('AOI_dbasins_unique', 'var') ~= 1 && exist('AOI_dbasins_unique_nr', 'var') ~= 1
        ...
        idx_area = find([AOI_dbasins_unique_stats.Area] > ...
            ((min_dbasins_stats_to_process)/(AOI_DEM.cellsize.^2)));
...
if ~isnan(AOI_dbasins_unique_nr)
    ...
    fprintf(['\tCalculating basin statistics for basins >%G m^2 (>%G km^2).\n',...
        'At basin # of %d: '], min_dbasins_stats_to_process, ...
        min_dbasins_stats_to_process/1e6, AOI_dbasins_unique_nr);
    for i = 1:AOI_dbasins_unique_nr
        ...
        AOI_FAC_dbasins_w = (AOI_FAC.*(AOI_FAC.cellsize.^2) > ...
            min_drainage_area_to_process) & (AOI_dbasins_current > 0);  % masks flow accum grid (above threshold)
        ...
        if length(AOI_STR_dbasins_unique_subset{i}.x) > 20
            ...
            idx0 = find(AOI_STR_slope_subset{i} <= min_str_gradient);
            ...
            % remove gradient less than minimum gradient defined in min_str_gradient
            idx0 = find(AOI_STR_slope_trunk_subset{i} <= min_str_gradient);
            ...
            AOI_STR_S_slopearea_dbasins{i} = slopearea(AOI_STR_streams_dbasins_unique{i},...
                AOI_mg, AOI_FAC, 'theta', abs(theta), ...
            ...
            shapeout_fn = sprintf('shapefiles%strunk%s%s_MS_trunk_ksn_db%d.shp', ...
                dir_sep, dir_sep, DEM_basename, i);
            shapeout_fn_out = sprintf('shapefiles%strunk%s%s_MS_trunk_ksn_db%d.out', ...
                dir_sep, dir_sep, DEM_basename, i);
            shapeout_all_fn = sprintf('shapefiles%strunk%s%s_MS_trunk_ksn_db%d.*', ...
                dir_sep, dir_sep, DEM_basename, i);
            if exist(shapeout_fn, 'file') ~= 2
                ...
                shapeout_fn_prj = sprintf('shapefiles%strunk%s%s_MS_trunk_ksn_db%d_proj.shp', ...
                    dir_sep, dir_sep, DEM_basename, i);
        ...
        if nnz(outlet)==1, AOI_STR_S_chiplot{i} = ...
                ...
                AOI_FAC, 'a0', min_drainage_area_to_process, ...
        ...
        if nnz(outlet)==1, AOI_STR_S_trunk_chiplot{i} = ...
                ...
                'a0', min_drainage_area_to_process, 'trunkstream', ...
        ...
        idx0 = find(grad <= min_str_gradient*10); grad(idx0) = []; area(idx0) = [];
        if length(area) >= 100
            %enough data points
            ...
            min_logspace = floor(log10(min_max_DA_fits(1)));
            max_logspace = ceil(log10(min_max_DA_fits(2)));
            % create min_max_DA_fits(3) log-spaced points between 10^min_logspace and 10^max_logspace:
            edges_log = logspace(min_logspace, max_logspace, min_max_DA_fits(3));
        ...
        if show_figs == 1 || show_figs == 2
            ...
            if exist(slope_area_plot_fname, 'file') ~= 2
                ...
                if no_plot == 0
                    ...
                    title_string = sprintf('DEM: %s, Basin #: %d, Trunk Stream (m/n = %0.2f)', ...
                        DEM_basename_no_underscore, i, AOI_STR_S_trunk_chiplot{i}.mn);
                    title(title_string, 'Fontsize', 16), grid;
                    %adding text: ksn, ks, theta, theta_ref, r2 and regression boundaries from SA plots
                    ...
                    grid on
                    hold off
                    export_fig(slope_area_plot_fname,quality_flag,'-pdf');
                end
            end
            ...
            if exist(slope_area_map_fname, 'file') ~= 2
                ...
                title_string = sprintf('%s: K_{s} adjusted with theta = %0.3f ', ...
                    DEM_basename_no_underscore, AOI_STR_S_slopearea_dbasins{i}.theta);
                title(title_string, 'Fontsize', 14), grid;
                colorbar
                ...
                axis([min(AOI_STR_dbasins_unique_subset{i}.x)-(xrange/15) ...
                    ...
                    max(AOI_STR_dbasins_unique_subset{i}.y)+(yrange/15)])
                hold off
                export_fig(slope_area_map_fname,quality_flag,'-pdf');
            end
        end
    ...
    fprintf('\n');
    save (DEM_STR_MAT_fname,'AOI_STR_S_trunk_chiplot','AOI_STR_S_chiplot', 'AOI_STR_S_slopearea_dbasins',...
        ...
        'AOI_dbasins_unique_nr', 'AOI_dbasins_unique', 'AOI_dbasins_stats', ...
        'AOI_STR_S_slopearea_dbasins_trunk', 'AOI_ks045_crop', ...
        'gof_logspace', 'AOI_STR_S_slopearea_dbasins_trunk_chitheta', '-v7.3');
end

if exist(DEM_STR_MAT_fname, 'file') == 2
    if exist('AOI_dbasins_unique_nr', 'var') ~= 1
        load(DEM_STR_MAT_fname)
    end
    for i = 1:AOI_dbasins_unique_nr
        no_plot = 0;
        %only calculate log area-log slope plot, if there are more than 20 elements in vector
        if length(AOI_STR_dbasins_unique_subset{i}.x) > 20
            ...
            % this will take a while
            %fprintf(1,'\tcreating mapstruct.\n');
            shapeout_fn = sprintf('shapefiles%strunk%s%s_MS_trunk_ksn_db%d.shp', dir_sep, dir_sep, DEM_basename, i);
            shapeout_fn_out = sprintf('shapefiles%strunk%s%s_MS_trunk_ksn_db%d.out', dir_sep, dir_sep, DEM_basename, i);
            shapeout_all_fn = sprintf('shapefiles%strunk%s%s_MS_trunk_ksn_db%d.*', dir_sep, dir_sep, DEM_basename, i);
            shapeout_fn_prj = sprintf('shapefiles%strunk%s%s_MS_trunk_ksn_db%d_proj.shp', dir_sep, dir_sep, DEM_basename, i);
...
if exist('AOI_dbasins_stats_min_area', 'var') ~= 0
    ...
    if exist(AOI_dbasins_stats_CNTR_csv_fname, 'file') ~= 2
        fprintf('\tWriting basin shapefiles\n');
        csvwrite_with_headers(AOI_dbasins_stats_CNTR_csv_fname,AOI_dbasins_stats_min_areab,headers,precision);
        csvwrite_with_headers(AOI_dbasins_stats_OUT_csv_fname,AOI_dbasins_stats_min_areab,headers,precision);
    end
    if exist(AOI_dbasins_stats_fname, 'file') ~= 2
        GRIDobj2geotiff(AOI_dbasins_stats, AOI_dbasins_stats_fname);
    end
    ...
    db_stats_shapeout_fn = strcat(DEM_basename_nodir, '_db_stats_CNTR.shp');
    db_stats_shapeout_fn_out = strcat(DEM_basename_nodir, '_db_stats_CNTR.out');
    db_stats_shapeout_all_fn = strcat(DEM_basename_nodir, '_db_stats_CNTR.*');
    db_stats_crt_fn = strcat(DEM_basename_nodir, '_db_stats_CNTR.crt');
    if exist(strcat(sprintf('shapefiles%s', dir_sep),db_stats_shapeout_fn), 'file') ~= 2
        ...
        string2write = sprintf(['<OGRVRTDataSource>\n  <OGRVRTLayer name=\"%s\">\n', ...
            ...
            '  </OGRVRTLayer>\n', '</OGRVRTDataSource>\n'], ...
            strcat(DEM_basename_nodir, '_db_stats_CNTR'), AOI_dbasins_stats_CNTR_csv_fname);
    ...
    eval([mv_cmd, ' ', db_stats_shapeout_all_fn, ' ', sprintf('shapefiles%s', dir_sep)]);
    
    % Write shapefile with outlet coordinates
    db_stats_shapeout_fn  = strcat(DEM_basename_nodir, '_db_stats_OUT.shp');
    db_stats_shapeout_fn_out  = strcat(DEM_basename_nodir, '_db_stats_OUT.out');
    db_stats_shapeout_all_fn  = strcat(DEM_basename_nodir, '_db_stats_OUT.*');
    db_stats_crt_fn = strcat(DEM_basename_nodir, '_db_stats_OUT.crt');
    if exist(strcat(sprintf('shapefiles%s', dir_sep),db_stats_shapeout_fn), 'file') ~= 2
        db_stats_FID = fopen(db_stats_crt_fn, 'w+');
        string2write = sprintf(['<OGRVRTDataSource>\n  <OGRVRTLayer name=\"%s\">\n', ...
            '    <SrcDataSource relativeToVRT=\"1\">%s</SrcDataSource>\n', ...
            ...
            '    <Field name="35DA_km2" type="Real" width="8" precision="7"/>\n', ...
            '  </OGRVRTLayer>\n', '</OGRVRTDataSource>\n'], ...
            strcat(DEM_basename_nodir, '_db_stats_OUT'), AOI_dbasins_stats_OUT_csv_fname);
        fwrite(db_stats_FID, string2write);
        fclose(db_stats_FID);
        eval([gdalsrsinfo_cmd, ' -o wkt ', DEM_fname, '> projection.prj']);
        eval([ogr2ogr_cmd, ' -s_srs projection.prj -t_srs projection.prj -f "ESRI Shapefile" ', ...
