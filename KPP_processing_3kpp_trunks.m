...
%
fprintf('Extracting knickpoints for trunk streams. At basin # of %d: ', number_of_basins);
for i = 1:number_of_basins
    fprintf('%d, ', i);
    if mod(i,10) == 0
        fprintf('\nExtracting knickpoints for trunk streams. At basin # of %d: ', number_of_basins);
    end
    ...
    current_tbasin_chiplot = AOI_STR_S_trunk_chiplot{i};
    current_tbasin_streamobj = AOI_STR_all_streams_trunk{i};
    
    idx_confluences = ~isnan(current_tbasin_chiplot.chi);  % sets cells that are NaN's to 0   (the confluences are where cells within .chi = NaN)
    ...
    tributaries_t = {};  % create empty cell array to store tributaries in
    ...
    chi_stored_t = {}; % erase previous data
    elev_stored_t = {}; % erase previous data
    eastingUTM11_stored_t = {}; % erase previous data
    northingUTM11_stored_t = {} ;% erase previous data
    upstream_DA_stored_t = {} ;% erase previous data
    distance_stored_t = {};
    ...
    for j = 1:(length(confluences)-1);
        tributaries_t{j} = [confluences(j):1:confluences(j+1)] ;
        ...
        % tributaries
        chi_stored_t{j} = current_tbasin_chiplot.chi(tributaries_t{j})  ;
        % Do the same thing for the .elev data
        elev_stored_t{j} = current_tbasin_chiplot.elev(tributaries_t{j});
        % Do the same thing for the .x data (easting UTM 11)
        eastingUTM11_stored_t{j} = current_tbasin_chiplot.x(tributaries_t{j})   ;
        % Do the same thing for the .y data (northing UTM 11)
        northingUTM11_stored_t{j} = current_tbasin_chiplot.y(tributaries_t{j}) ;
        % drainge area info
        upstream_DA_stored_t{j} = current_tbasin_chiplot.area(tributaries_t{j}) ;
        % distance infor
        distance_stored_t{j} = current_tbasin_chiplot.distance(tributaries_t{j}) ;
        % done
    end
    ...
    for j = 1:length(tributaries_t);
        % Mark cells for short tributaries in the stored elevation cell array (same
        % cells as chi, .x, .y, ect..
        if length(chi_stored_t{j}) < min_trib_size ; % measure the length of the chi vector for each tributary. if that vector is too small we ignore that stream segment
            chi_stored_t{j} = -9999 ; % some ridiculous number you can use as a marker indicating a cell that refers to a stream that is too small that we don't care about
            elev_stored_t{j} = -9999;
            eastingUTM11_stored_t{j} = -9999;
            northingUTM11_stored_t{j} = -9999;
            upstream_DA_stored_t{j} = -9999;
            distance_stored_t{j} = -9999;
        end
    end
    ...
    % "chi_stored" (for example) that we marked -9999
    for j = 1:length(tributaries_t);
        if chi_stored_t{j} == -9999;
            chi_stored_t{j} = [];
            elev_stored_t{j} = [];
            eastingUTM11_stored_t{j} = [];
            northingUTM11_stored_t{j} = [];
            upstream_DA_stored_t{j} = [];
            distance_stored_t{j} = [];
        end
    end
    ...
    elev_stored_trimmed = elev_stored_t(~cellfun(@isempty, elev_stored_t))   ;
    chi_stored_trimmed = chi_stored_t(~cellfun(@isempty, chi_stored_t))    ;
    eastingUTM11_stored_trimmed = eastingUTM11_stored_t(~cellfun(@isempty, eastingUTM11_stored_t))    ;
    northingUTM11_stored_trimmed = northingUTM11_stored_t(~cellfun(@isempty, northingUTM11_stored_t)) ;
    upstream_DA_stored_trimmed = upstream_DA_stored_t(~cellfun(@isempty, upstream_DA_stored_t)) ;
    distance_stored_trimmed = distance_stored_t(~cellfun(@isempty, distance_stored_t)) ;
    ...
    elev_stored_trimmed_basin_t{i} = elev_stored_trimmed ;
    chi_stored_trimmed_basin_t{i} = chi_stored_trimmed ;
    eastingUTM11_stored_trimmed_basin_t{i} = eastingUTM11_stored_trimmed ;
    northingUTM11_stored_trimmed_basin_t{i} = northingUTM11_stored_trimmed ;
    upstream_DA_stored_trimmed_basin_t{i} = upstream_DA_stored_trimmed ;
    distance_stored_trimmed_basin_t{i} = distance_stored_trimmed ;
...
fprintf('Postprocessing knickpoints for trunk streams. At basin # of %d: ', number_of_basins);
for i = 1:number_of_basins
    fprintf('%d, ', i);
    if mod(i,10) == 0
        fprintf('\nPostprocessing knickpoints for trunk streams. At basin # of %d: ', number_of_basins);
    end
    
    chi_stored_current_t = chi_stored_trimmed_basin_t{i}; % chi info for each tributary within the stream {i}
    elev_stored_current_t  = elev_stored_trimmed_basin_t{i}; % elev info for each tributary within the stream {i}
    eastingUTM11_stored_current_t  = eastingUTM11_stored_trimmed_basin_t{i}; % ect..
    northingUTM11_stored_current_t  = northingUTM11_stored_trimmed_basin_t{i};
    upstream_DA_stored_current_t  = upstream_DA_stored_trimmed_basin_t{i};
    distance_stored_current_t = distance_stored_trimmed_basin_t{i};
    ...
    kp_magnitude_matrix_final_t = {};
    kp_chi_length_final_t = {};
    kp_face_slope_chi_final_t = {};
    kp_up_da_t = {};
    kp_distance_t = {};
    kp_easting_t = {};
    kp_northing_t = {};
    ...
    kp_northing_bases_t = {};
    kp_chi_bases_t = {};
    kp_elevation_bases_t = {};
    
    % Now the stream catchment is organized so each cell  within array '_strored_trimmed' is a tributary we want to look at
    ...
    number_of_tributaries = length(elev_stored_current_t);
    
    for j = 1:number_of_tributaries
        % get the information for the tributary
        current_chi = chi_stored_current_t{j};
        ...
        current_elev = elev_stored_current_t{j};
        current_easting = eastingUTM11_stored_current_t{j};
        current_northing = northingUTM11_stored_current_t{j};
        current_upDA = upstream_DA_stored_current_t{j};
        current_distance = distance_stored_current_t{j};
        ...
        mx_b = polyfit(current_chi_resampled,current_elev_resampled,1); % calculate linear regression
        blf_y_current_tribs = (mx_b(1)*current_chi_resampled + mx_b(2)); % get the y coordinates for the trendline
        
        % save slope of chiplot
        beta_current_trib = mx_b(1);
...
%         end
        ...
        for t = 1:length(p)
            ...
            if sig_kps_chi(m-1) - sig_kps_chi(m) < lumping_search_distance && kp_elev_bases(m) < kp_elev_bases(m-1)
        ...
        for k = 1:length(sig_kps_cells_lumped); % for how many potential knickpoints we still have
            if kp_magnitude_matrix_lumped_equispaced(k) < min_kp_size2 % specify a 2nd minimum knickpoint size if you'd like (after summing closely spaced knickpoints together)
        ...
        % elev/chi is the slope of the chi plot, or steepness
        
        % detended elev/chi would be the steepness anomoly
        
        kp_length_chi = current_chi_resampled(sig_kps_cells_lumped_filtered)- current_chi_resampled(sig_kps_bases_lumped_filtered);
        % calculate chi length of knickpoint
        
        kp_face_slope_chi = kp_magnitude_matrix_lumped_filtered  ./ kp_length_chi ; % divide drop in elevation by change in chi
        ...
        for k = 1:length(kp_length_chi) % for the number of potential knickpoitns we still have
            if kp_face_slope_chi(k) < min_kp_slope;% Parmeter line 105
                ...
                kp_length_chi(k) = NaN;  % mark cells that belong to too gradual knickpoint
                kp_face_slope_chi(k) = NaN;  % mark cells that belong to too gradual knickpoint
        ...
        sig_kps_cells_final_t = sig_kps_cells_lumped_filtered(idx_kps_tribs_4);  % index out knickpoint lips to knickpoints that were to gradual
        
        idx_bases_tribs_4= ~isnan(sig_kps_bases_lumped_filtered);
        sig_kps_bases_final_t = sig_kps_bases_lumped_filtered(idx_bases_tribs_4); % respective bases
        
        idx_magnitude_matrix_3 = ~isnan(kp_magnitude_matrix_lumped_filtered);
        kp_magnitude_matrix_final_t{j}  = kp_magnitude_matrix_lumped_filtered(idx_magnitude_matrix_3);  % respective knickpoint magnitudes
        
        idx_kp_chi_length = ~isnan(kp_length_chi);
        kp_chi_length_final_t{j} = kp_length_chi(idx_kp_chi_length );                       % respective knickpoint lengths (in units of chi)
        
        idx_kp_face_slope_dist_space_detrended  = ~isnan(kp_face_slope_chi);
        kp_face_slope_chi_final_t{j}= kp_face_slope_chi(idx_kp_face_slope_dist_space_detrended);   % respective knickpoint slopes  detrended elev/chi
        ...
        kp_cells_chi_equispaced = current_chi_resampled(sig_kps_cells_final_t); % chi coordinates for knickpoint lips
        kp_cells_bases_chi_equispaced = current_chi_resampled(sig_kps_bases_final_t); % chi coordinates for knickpoint bases
        ...
        knickpoint_relief_t{j} = current_kp_lip_elev - current_kp_bases_elev;

        
        % knickpoint lips
        kp_up_da_t{j} = current_upDA(kp_cells);  % drainage area upstream of knickpoint
        kp_distance_t{j} = current_distance(kp_cells); % knickpoint distance upstream
        kp_easting_t{j} = current_easting(kp_cells);
        kp_northing_t{j} = current_northing(kp_cells);
        kp_chi_t{j} = current_chi(kp_cells); % chi coordinate
        kp_elev_t{j} = current_elev(kp_cells); % elevation of knickpoint
        
        % knickpoint bases
        kp_up_da_bases_t{j} = current_upDA(kp_cells_bases);  % drainage area upstream of knickpoint
        kp_distance_bases_t{j} = current_distance(kp_cells_bases); % knickpoint distance upstream
        kp_easting_bases_t{j} = current_easting(kp_cells_bases);
        kp_northing_bases_t{j} = current_northing(kp_cells_bases);
        kp_chi_bases_t{j} = current_chi(kp_cells_bases); % chi coordinate
        kp_elevation_bases_t{j} = current_elev(kp_cells_bases); % elevation of knickpoint
        
        
        % Save all the important attributes in cell arrays, each cell representing the current tributary analyzed
        % Knickpoint lips
        kp_up_da_stored_lips_t{i} = kp_up_da_t;
        kp_distance_stored_lips_t{i} = kp_distance_t;
        kp_easting_stored_lips_t{i} = kp_easting_t;
        kp_northing_stored_lips_t{i} = kp_northing_t;
        kp_chi_stored_lips_t{i} = kp_chi_t;
        kp_elev_stored_lips_t{i} = kp_elev_t;
        
        % Knickpoint Bases
        kp_up_da_stored_bases_t{i} = kp_up_da_bases_t;
        kp_distance_stored_bases_t{i} = kp_distance_bases_t;
        kp_easting_stored_bases_t{i} = kp_easting_bases_t;
        kp_northing_stored_bases_t{i} = kp_northing_bases_t;
        kp_chi_stored_bases_t{i} = kp_chi_bases_t;
        kp_elev_stored_bases_t{i} = kp_elevation_bases_t;
        
        % Knickpoint measurements
        kp_magnitude_matrix_stored_t{i} = kp_magnitude_matrix_final_t;
        kp_chi_length_stored_t{i} = kp_chi_length_final_t;
        kp_face_slope_chi_stored_t{i} = kp_face_slope_chi_final_t;
        kp_relief_stored_t{i} = knickpoint_relief_t;
        
        % save slope of current tributary
        beta_all_tribs_t{j} = beta_current_trib;  % for each tributary
        beta_all_tribs_all_basins_t{i} = beta_all_tribs_t;  % for each basin
    end
end
...
number_of_basins2 = length(kp_elev_stored_lips_t);
for i = 1:number_of_basins2 % % for all stream basins
    %for all the knickpoint lips
    current_basin_elevation = kp_elev_stored_lips_t{i};
    current_basin_chi = kp_chi_stored_lips_t{i};
    current_basin_easting = kp_easting_stored_lips_t{i};
    current_basin_northing = kp_northing_stored_lips_t{i};
    current_basin_upstream_DA = kp_up_da_stored_lips_t{i};
    current_basin_distance = kp_distance_stored_lips_t{i};
    
    % measurements between lips and bases
    current_basin_magnitude = kp_magnitude_matrix_stored_t{i};
    current_basin_kp_slope_detrend = kp_face_slope_chi_stored_t{i};
    current_basin_kp_length_distance = kp_chi_length_stored_t{i};
    current_basin_kp_relief = kp_relief_stored_t{i};
    % for knickpoint bases
    current_basin_elevation_bases = kp_elev_stored_bases_t{i};
    current_basin_chi_bases = kp_chi_stored_bases_t{i};
    current_basin_easting_bases = kp_easting_stored_bases_t{i};
    current_basin_northing_bases = kp_northing_stored_bases_t{i};
    current_basin_upstream_DA_bases = kp_up_da_stored_bases_t{i};
    current_basin_distance_bases = kp_distance_stored_bases_t{i};
    ...
    for j = 1:length(current_basin_elevation);  % for each tributary, vertically concatinate all the knickpoints from each tributary within the basin
        ...
        current_basins_all_kps_relief = vertcat(current_basins_all_kps_relief,current_basin_kp_relief{j});
        %bases
    ...
    all_basins_all_kps_relief{i} = current_basins_all_kps_relief;
...
trunk_kp_easting = [];
trunk_kp_northing = [];
trunk_kp_upstream_DA = [];
trunk_kp_distance = [];
%measurements
trunk_kp_slope_detrend = [];
trunk_kp_length_distance = [];
trunk_kp_magnitude = [];
trunk_kp_relief = [];
% bases
trunk_kp_elevations_bases = [];
trunk_kp_chi_bases = [];
trunk_kp_easting_bases = [];
trunk_kp_northing_bases = [];
trunk_kp_upstream_DA_bases = [];
trunk_kp_distance_bases = [];


% concatinate all the kp lists from each basin together into 1 list
for i = 1:number_of_basins2 ;% % for all stream basin
    % lips
    trunk_kp_elevations = vertcat(trunk_kp_elevations,all_basins_all_kps_elevation{i}); % 6
    trunk_kp_chi = vertcat(trunk_kp_chi,all_basins_all_kps_chi{i}); %5
    trunk_kp_easting = vertcat(trunk_kp_easting,all_basins_all_kps_easting{i}); %9
    trunk_kp_northing = vertcat(trunk_kp_northing,all_basins_all_kps_northing{i}); %10
    trunk_kp_upstream_DA = vertcat(trunk_kp_upstream_DA,all_basins_all_kps_upstream_DA{i}); %11
    trunk_kp_distance = vertcat(trunk_kp_distance,all_basins_all_kps_distance{i}); %12
    % measurements
    trunk_kp_magnitude = vertcat(trunk_kp_magnitude,all_basins_all_kps_magnitude{i}); %7
    trunk_kp_slope_detrend = vertcat(trunk_kp_slope_detrend,all_basins_all_kps_slope_detrend{i}); % 15
    trunk_kp_length_distance = vertcat(trunk_kp_length_distance,all_basins_all_kps_length_distance{i}); %16
    trunk_kp_relief = vertcat(trunk_kp_relief,all_basins_all_kps_relief{i}); %8
    % bases
    trunk_kp_elevations_bases = vertcat(trunk_kp_elevations_bases,all_basins_all_kps_elevation_bases{i}); %6
    trunk_kp_chi_bases = vertcat(trunk_kp_chi_bases,all_basins_all_kps_chi_bases{i}); %5
    trunk_kp_easting_bases = vertcat(trunk_kp_easting_bases,all_basins_all_kps_easting_bases{i}); %9
    trunk_kp_northing_bases = vertcat(trunk_kp_northing_bases,all_basins_all_kps_northing_bases{i}); %10
    trunk_kp_upstream_DA_bases = vertcat(trunk_kp_upstream_DA_bases,all_basins_all_kps_upstream_DA_bases{i}); %11
    trunk_kp_distance_bases = vertcat(trunk_kp_distance_bases,all_basins_all_kps_distance_bases{i}); %12
end
...
for i = 1:number_of_basins2 ;% % for all stream basins
    current_basin_elevation = kp_elev_stored_lips_t{i};
    beta_list_current_basin = beta_all_tribs_all_basins_t{i};
...
%
total_number_of_kps = length(trunk_kp_elevations);
...
for k = 1 : total_number_of_kps
    sgolay_window_size(k) = smoothing_window;   % (17)
    max_kp_search_dist(k) = lumping_search_distance ;   % (18)
    min_kp_size_pre_combination(k)= min_kp_size1 ;    % (19)
    min_kp_size_post_combination(k)= min_kp_size2;     % (20)
    min_kp_slope_detrended (k) = min_kp_slope ;   % (21)
    min_trib_size_cells(k) = min_trib_size; % (22)
end
%
% CALCULATE REAL SLOPE (Y/X in landscape)
% Slope raw
dy = trunk_kp_elevations - trunk_kp_elevations_bases; % (m elevation drop)
dx = trunk_kp_distance - trunk_kp_distance_bases; % (m distance upstream)
kp_slope_raw = dy./dx; % 13
% slope normalized for drainage area
kp_slope_detrend_elv_dist = trunk_kp_magnitude./dx; %14
...
% knickpoint lips
Knickpoint_Database_Lips_t = horzcat(kp_id_number,stream_id,all_trib_ids, ...
    all_beta_list, trunk_kp_chi,trunk_kp_elevations,trunk_kp_magnitude, trunk_kp_relief,...
    trunk_kp_easting, trunk_kp_northing ,trunk_kp_upstream_DA ,trunk_kp_distance, ...
    kp_slope_raw, kp_slope_detrend_elv_dist, trunk_kp_slope_detrend, ...
    trunk_kp_length_distance, sgolay_window_size, max_kp_search_dist, ...
...
%the knickpoints)
Knickpoint_Database_Bases_t = horzcat(kp_id_number,stream_id,all_trib_ids,...
    all_beta_list, trunk_kp_chi_bases,trunk_kp_elevations_bases, ...
    trunk_kp_magnitude, trunk_kp_relief,trunk_kp_easting_bases, trunk_kp_northing_bases, ...
    trunk_kp_upstream_DA_bases, trunk_kp_distance_bases, kp_slope_raw, ...
    kp_slope_detrend_elv_dist, trunk_kp_slope_detrend, ...
    trunk_kp_length_distance, sgolay_window_size, max_kp_search_dist, ...
    min_kp_size_pre_combination, min_kp_size_post_combination, ...
    min_kp_slope_detrended, min_trib_size_cells);
...
txt = sprintf('%s, ',hdr{:});
txt(end)='';
kp_lips_fn = strcat(DEM_basename_nodir, '_kp_lips_trunk.csv');
dlmwrite(kp_lips_fn,txt,'');
dlmwrite(kp_lips_fn,Knickpoint_Database_Lips_t,'-append','delimiter',',', 'precision', '%.5f');

kp_bases_fn = strcat(DEM_basename_nodir, '_kp_bases_trunk.csv');
dlmwrite(kp_bases_fn,txt,'');
dlmwrite(kp_bases_fn,Knickpoint_Database_Bases_t,'-append','delimiter',',', 'precision', '%.5f');
...
% formats with gdal
kp_lips_shapeout_fn  = strcat(DEM_basename_nodir, '_kp_lips_trunk.shp');
kp_lips_shapeout_fn_out  = strcat(DEM_basename_nodir, '_kp_lips_trunk.out');
kp_lips_shapeout_fn_all = strcat(DEM_basename_nodir, '_kp_lips_trunk.*');
kp_lips_crt_fn = strcat(DEM_basename_nodir, '_kp_lips_trunk.crt');
...
string2write = sprintf(['<OGRVRTDataSource>\n  <OGRVRTLayer name=\"%s\">\n', ...
    ...
    '  </OGRVRTLayer>\n', '</OGRVRTDataSource>\n'], strcat(DEM_basename_nodir, '_kp_lips_trunk'), ...
    kp_lips_fn);
fwrite(lips_FID, string2write);
fclose(lips_FID);
if exist(strcat(sprintf('shapefiles%s', dir_sep), kp_lips_shapeout_fn), 'file') ~= 2
    eval([gdalsrsinfo_cmd, ' -o wkt ', DEM_fname, '> projection.prj']);
...
if exist(kp_lips_shapeout_fn_all, 'file') ~= 2
    eval([mv_cmd, ' ', kp_lips_shapeout_fn_all, ' ', sprintf('shapefiles%s', dir_sep)]);
end

kp_bases_shapeout_fn = strcat(DEM_basename_nodir, '_kp_bases_trunk.shp');
kp_bases_shapeout_fn_out = strcat(DEM_basename_nodir, '_kp_bases_trunk.out');
kp_bases_shapeout_fn_all = strcat(DEM_basename_nodir, '_kp_bases_trunk.*');
kp_bases_crt_fn = strcat(DEM_basename_nodir, '_kp_bases_trunk.crt');
...
string2write = sprintf(['<OGRVRTDataSource>\n  <OGRVRTLayer name=\"%s\">\n', ...
    ...
    '    <Field name="22strea_sz" type="Real" width="8" precision="7"/>\n', ...
    '  </OGRVRTLayer>\n', '</OGRVRTDataSource>\n'], ...
    strcat(DEM_basename_nodir, '_kp_bases_trunk'), kp_bases_fn);
fwrite(bases_FID, string2write);
fclose(bases_FID);
if exist(strcat(sprintf('shapefiles%s', dir_sep), kp_bases_shapeout_fn), 'file') ~= 2
    eval([gdalsrsinfo_cmd, ' -o wkt ', DEM_fname, '> projection.prj']);
    eval([ogr2ogr_cmd, ' -s_srs projection.prj -t_srs projection.prj -f "ESRI Shapefile" ', ...
        kp_bases_shapeout_fn, ' ', kp_bases_crt_fn, ' 2> ', kp_bases_shapeout_fn_out]);
end
if exist(kp_lips_shapeout_fn_all, 'file') ~= 2
    eval([mv_cmd, ' ', kp_bases_shapeout_fn_all, ' ', sprintf('shapefiles%s', dir_sep)]);
end

% verify if crt file is correctly formatted:
% ogrinfo -ro -al SCIsw_LiDAR1m_dem_kp_lips.crt
