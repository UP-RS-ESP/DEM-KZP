...
for k = 1:length(stream_order)
    ...
    fprintf('step 4 of 4: area-slope and chi-plot processing for stream order basins (STO = %d)\n', stream_order2use);
    AOI_STO_dbasins_stats_CNTR_csv_fname = sprintf('%s_STO_%02d_db_stats_CNTR.csv', DEM_basename_nodir, stream_order2use);
    AOI_STO_dbasins_stats_OUT_csv_fname = sprintf('%s_STO_%02d_db_stats_OUT.csv', DEM_basename_nodir, stream_order2use);
    if exist(strcat(sprintf('shapefiles%sSTO%s', dir_sep, dir_sep), AOI_STO_dbasins_stats_CNTR_csv_fname), 'file') == 2 && ...
            exist(strcat(sprintf('shapefiles%sSTO%s', dir_sep, dir_sep), AOI_STO_dbasins_stats_OUT_csv_fname), 'file') == 2
        continue
    end
    
    [AOI_STO_N, AOI_STO_N_outlet] = drainagebasins(AOI_FD, AOI_STO, stream_order2use);
    AOI_STO_N_fname = sprintf('geotiff%s%s_STO_%02d.tif', dir_sep, DEM_basename_nodir, stream_order2use);
    ...
    if exist('AOI_STO_N_unique', 'var') ~= 1
        ...
        for i = 1:counter
            ...
            AOI_STO_msk_N = STREAMobj(AOI_FD, AOI_FAC_msk > (min_drainage_area_to_process./(AOI_FAC_msk.cellsize^2)));
            ...
            if length(AOI_STO_msk_N.x) > 20
                ...
                idx0 = find(AOI_STO_STR_slope_subset{i} <= min_str_gradient);
                ...
                % remove gradient less than minimum gradient defined in min_str_gradient
                idx0 = find(AOI_STO_STR_slope_trunk_subset{i} <= min_str_gradient);
                ...
                if nnz(outlet)==1, AOI_STO_STR_S_chiplot{i} = chiplot(AOI_STO_msk_N, AOI_mg, AOI_FAC, 'a0', min_drainage_area_to_process, 'fitto', 'all', 'plot', false);
                ...
                outlet = sum(M,2) == 0 & sum(M,1)'~=0;
                if nnz(outlet)==1, AOI_STO_STR_S_trunk_chiplot{i} = chiplot(AOI_STO_msk_N, AOI_mg,AOI_FAC, 'a0', min_drainage_area_to_process, 'trunkstream', AOI_STO_trunk_N(i), 'fitto', 'ts', 'plot', false);
                ...
                idx0 = find(grad <= min_str_gradient);
                grad(idx0) = []; area(idx0) = [];
                if length(area) >= 10
                    %enough data points
                ...
                elseif length(area) < 10
                    AOI_STO_dbasins_stats(i,:) = [double(AOI_STO_N_unique(i)), utm_x_centroid, utm_y_centroid, BasinO_X, BasinO_Y, double(AOI_STO_STR_S_slopearea{i}.ks), ...
                        double(AOI_STO_STR_S_slopearea{i}.theta), double(AOI_STO_STR_S_slopearea_trunk{i}.ks), double(AOI_STO_STR_S_slopearea_trunk{i}.theta), ...
                        NaN, NaN, NaN, NaN, NaN, NaN, NaN, NaN, NaN, NaN, NaN, NaN, NaN, NaN, NaN, NaN, NaN, NaN, NaN, NaN, max(AOI_STO_STR_S_slopearea{i}.a), max(AOI_STO_STR_S_slopearea{i}.a)/1e6];
                end
                clear area grad xData yData xData_logspace yData_logspace wData_logspace p_logspace_a p_logspace_b p_rweight_a p_rweight_b xData_median yData_median yData_std
            else
                utm_x_centroid = AOI_x(round(AOI_STO_N_stats(i).Centroid(2)), round(AOI_STO_N_stats(i).Centroid(1)));
                utm_y_centroid = AOI_y(round(AOI_STO_N_stats(i).Centroid(2)), round(AOI_STO_N_stats(i).Centroid(1)));
        ...
        if MISC_FILES == 1
            ...
            fname2save = sprintf('%s_STO_%02d_MS.mat', DEM_basename_nodir, stream_order2use);
        ...
        if exist(strcat(sprintf('shapefiles%sSTO%s', dir_sep, dir_sep), AOI_STO_dbasins_stats_OUT_csv_fname), 'file') ~= 2
        ...
        db_stats_shapeout_fn = sprintf('%s_STO_%02d_db_stats_CNTR.shp', DEM_basename_nodir, stream_order2use);
        db_stats_shapeout_fn_out = sprintf('%s_STO_%02d_db_stats_CNTR.out', DEM_basename_nodir, stream_order2use);
        db_stats_shapeout_all_fn  = sprintf('%s_STO_%02d_db_stats_CNTR.*', DEM_basename_nodir, stream_order2use);
        if exist(strcat(sprintf('shapefiles%sSTO%s', dir_sep, dir_sep), db_stats_shapeout_fn), 'file') ~= 2
            db_stats_crt_fn = sprintf('%s_STO_%02d_db_stats_CNTR.crt', DEM_basename_nodir, stream_order2use);
            ...
            string2write = sprintf(['<OGRVRTDataSource>\n  <OGRVRTLayer name=\"%s\">\n', ...
                ...
                '  </OGRVRTLayer>\n', '</OGRVRTDataSource>\n'], ...
                sprintf('%s_STO_%02d_db_stats_CNTR', DEM_basename_nodir, stream_order2use), AOI_STO_dbasins_stats_CNTR_csv_fname);
            ...
            eval([mv_cmd, ' ', db_stats_shapeout_all_fn, ' ', sprintf('shapefiles%sSTO%s', dir_sep, dir_sep)]);
            eval([remove_cmd, ' ', sprintf('shapefiles%sSTO%s%s', dir_sep, dir_sep, db_stats_crt_fn)]);
        end
        
        % Write shapefile with outlet coordinates
        db_stats_shapeout_fn  = sprintf('%s_STO_%02d_db_stats_OUT.shp', DEM_basename_nodir, stream_order2use);
        db_stats_shapeout_fn_out  = sprintf('%s_STO_%02d_db_stats_OUT.out', DEM_basename_nodir, stream_order2use);
        db_stats_shapeout_all_fn  = sprintf('%s_STO_%02d_db_stats_OUT.*', DEM_basename_nodir, stream_order2use);
        if exist(strcat(sprintf('shapefiles%sSTO%s', dir_sep, dir_sep), db_stats_shapeout_fn), 'file') ~= 2
            db_stats_crt_fn = sprintf('%s_STO_%02d_db_stats_OUT.crt', DEM_basename_nodir, stream_order2use);
            db_stats_FID = fopen(db_stats_crt_fn, 'w+');
            string2write = sprintf(['<OGRVRTDataSource>\n  <OGRVRTLayer name=\"%s\">\n', ...
                '    <SrcDataSource relativeToVRT=\"1\">%s</SrcDataSource>\n', ...
                '    <GeometryType>wkbPoint</GeometryType>\n', ...
                ...
                '    <Field name="35DA_km2" type="Real" width="8" precision="7"/>\n', ...
                '  </OGRVRTLayer>\n', '</OGRVRTDataSource>\n'], ...
                sprintf('%s_STO_%02d_db_stats_OUT', DEM_basename_nodir, stream_order2use), AOI_STO_dbasins_stats_OUT_csv_fname);
            fwrite(db_stats_FID, string2write);
            fclose(db_stats_FID);
            eval([gdalsrsinfo_cmd, ' -o wkt ', DEM_fname, '> projection.prj']);
            eval([ogr2ogr_cmd, ' -s_srs projection.prj -t_srs projection.prj -f "ESRI Shapefile" ', ...
...
close all
fprintf('\nfinished with KPP_topometrics_v1.m\n');
