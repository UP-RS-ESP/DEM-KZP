...
%
fprintf(1,'KZP identifying knickzones step 3 of 4: Extracting knickpoints for trunk streams.\nAt basin # of %d: ', number_of_basins);
for i = 1:number_of_basins
    ...
    for j = 1:length(tributaries_t);
        ...
        if length(chi_stored_t{j}) < KZP_parameters.min_trib_size ; % measure the length of the chi vector for each tributary. if that vector is too small we ignore that stream segment
            chi_stored_t{j} = NaN ;
            elev_stored_t{j} = NaN;
            eastingUTM11_stored_t{j} = NaN;
            northingUTM11_stored_t{j} = NaN;
            upstream_DA_stored_t{j} = NaN;
            distance_stored_t{j} = NaN;
        end
    end
...
fprintf('Postprocessing knickpoints for trunk streams.\nAt basin # of %d: ', number_of_basins);
for i = 1:number_of_basins
    fprintf('%d, ', i);
    if mod(i,10) == 0
        fprintf('\nAt basin # of %d: ', number_of_basins);
...
%%        Second Filter
        ...
        kp_chi_length_final_t{j} = kp_length_dist(idx_kp_chi_length );                       % respective knickpoint lengths (in units of chi)

        idx_kp_face_slope_dist_space_detrended  = ~isnan(kp_face_slope_d);
        kp_face_slope_chi_final_t{j}= kp_face_slope_d(idx_kp_face_slope_dist_space_detrended);   % respective knickpoint slopes  detrended elev/chi
...
txt = sprintf('%s, ',hdr{:});
txt(end)='';
kp_lips_fn = strcat(KZP_parameters.DEM_basename_nodir, '_kp_lips_trunk.csv');
dlmwrite(kp_lips_fn,txt,'');
dlmwrite(kp_lips_fn,Knickpoint_Database_Lips_t,'-append','delimiter',',', 'precision', '%.5f');

kp_bases_fn = strcat(KZP_parameters.DEM_basename_nodir, '_kp_bases_trunk.csv');
...
if exist('projection.prj', 'file') ~= 2
    eval([gdalsrsinfo_cmd, ' -o wkt ', KZP_parameters.DEM_fname, '> projection.prj']);
end
% Create .vrt file to convert .csv to shapefile or other vector-file
% formats with gdal
kp_lips_shapeout_fn  = strcat(KZP_parameters.DEM_basename_nodir, '_kp_lips_trunk.shp');
kp_lips_shapeout_fn_out  = strcat(KZP_parameters.DEM_basename_nodir, '_kp_lips_trunk.out');
kp_lips_shapeout_fn_all = strcat(KZP_parameters.DEM_basename_nodir, '_kp_lips_trunk.*');
kp_lips_crt_fn = strcat(KZP_parameters.DEM_basename_nodir, '_kp_lips_trunk.crt');
...
string2write = sprintf(['<OGRVRTDataSource>\n  <OGRVRTLayer name=\"%s\">\n', ...
    ...
    '  </OGRVRTLayer>\n', '</OGRVRTDataSource>\n'], strcat(KZP_parameters.DEM_basename_nodir, '_kp_lips_trunk'), ...
    kp_lips_fn);
...
kp_bases_shapeout_fn_out = strcat(KZP_parameters.DEM_basename_nodir, '_kp_bases_trunk.out');
kp_bases_shapeout_fn_all = strcat(KZP_parameters.DEM_basename_nodir, '_kp_bases_trunk.*');
kp_bases_crt_fn = strcat(KZP_parameters.DEM_basename_nodir, '_kp_bases_trunk.crt');
