...
KZP_parameters.min_kp_size2_relief,KZP_parameters.kp_relief_filter_option,KZP_parameters.kp_magnitude_filter_option,KZP_parameters.min_kp_slope,KZP_parameters.min_trib_size,...
KZP_parameters.min_drainage_area_to_process,KZP_parameters.theta_ref,ks_chiplot,ksn_SA_store,theta_bf,kz_up_da_c_basin_lips,ks_chiplot_all,...
...
% if knickzones were located
    for i = 1:number_of_basins
        ...
        for k = 1:length(current_kz_lips_elevation)
            if KZP_parameters.kp_magnitude_filter_option ==1 % used knickzone magnitude
            plot(current_kz_lips_chi(k),current_kz_lips_elevation(k),...
                'k.','MarkerSize',(((current_kz_magnitude(k))^KZP_parameters.kp_plot_size)+10))
            end

            if KZP_parameters.kp_relief_filter_option == 1 % used knickzone relief
            plot(current_kz_lips_chi(k),current_kz_lips_elevation(k),...
                'k.','MarkerSize',(((current_kz_relief(k))^KZP_parameters.kp_plot_size)+10))
            end
        end
        ...
        theta_ref_string = [' Ref Concavity = ', num2str(KZP_parameters.theta_ref)];
        ...
        saveas(H3,[pwd '/maps/',chiplot_fig_name,'.png']);
        ...
        smoothing_window_str = [' Smoothing Window = ', num2str(KZP_parameters.smoothing_window),' cells'];
        text(0,max(current_basin_elev_values{1}),smoothing_window_str);
        lumping_window_str = [' lumping Window = ', num2str(KZP_parameters.lumping_distance_upstream),' m'];
        text(0,max(current_basin_elev_values{1})-(max(current_basin_elev_values{1})/20),lumping_window_str);
        min_kz1_str = ['Min Kz 1 = ', num2str(KZP_parameters.min_kp_size1),' m'];
        text(0,max(current_basin_elev_values{1})-(max(current_basin_elev_values{1})/10),min_kz1_str );
        
        if KZP_parameters.kp_magnitude_filter_option == 1
            min_kz2_str  = [' Min Kz 2 = ', num2str(KZP_parameters.min_kp_size2_magnitude)];
            text(0,max(current_basin_elev_values{1})-(max(current_basin_elev_values{1})/7),min_kz2_str );
        end
        
        if KZP_parameters.kp_relief_filter_option == 1;
            min_kz2_str  = [' Min Kz 2 = ', num2str(KZP_parameters.min_kp_size2_relief),' m'];
            text(0,max(current_basin_elev_values{1})-(max(current_basin_elev_values{1})/7),min_kz2_str );
        end
        ...
        % need to add path to directory where we store figures
        saveas(H4,[pwd '/maps/',long_prof_filename,'.png']);
        % save figure in directory
        clf
        ...
        current_wd = pwd; % save the working directory
        ...
        %give filename
        cd([current_wd,KZP_parameters.dir_sep, KZ_databases_csv_dir])
        ...
        dlmwrite([current_wd,KZP_parameters.dir_sep,KZ_databases_csv_dir,KZP_parameters.dir_sep, kp_lips_fn],Knickpoint_Database_Lips_CB,'-append','delimiter',',', 'precision', '%.5f','roffset',1);
        ...
        % write .csv file containing knickzone lip information for
        % current parameters
        dlmwrite([current_wd,KZP_parameters.dir_sep,KZ_databases_csv_dir,KZP_parameters.dir_sep,kp_bases_fn],Knickpoint_Database_Bases_CB,'-append','delimiter',',', 'precision', '%.5f','roffset',1);
        
        cd(current_wd); % return to working directory
    end
end
...
% write .csv file containing knickzone lip information for
% current parameters (need one row offset for the header)
dlmwrite([current_wd,KZP_parameters.dir_sep,KZ_databases_csv_dir,KZP_parameters.dir_sep,kp_lips_fn],Knickpoint_Database_Lips,'-append','delimiter',',', 'precision', '%.5f','roffset',1);
...
% write .csv file containing knickzone lip information for
% current parameters
dlmwrite([current_wd,KZP_parameters.dir_sep,KZ_databases_csv_dir, KZP_parameters.dir_sep,kp_bases_fn],Knickpoint_Database_Bases,'-append','delimiter',',', 'precision', '%.5f','roffset',1);
